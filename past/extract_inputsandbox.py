...
#This script is used to extract from this file the list of input files (input sand box) of the job
#-Robin Apirl 6,2017
#--------------------------------------------------------------------------------
...
def main(_):
    ...
    with open("./"+args.filename,'rt') as file:
        ...
        for _ in list:
            ...
            if re.match(r'^[#]',line):
                continue
            if extract_rm(line, r'(transfer_input_files=)(.*)', queue):
                output['transfer_input_files'] = extract_rm(line, r'(transfer_input_files=)(.*)', queue)
                continue
            if extract_rm(line, r'(input=)(.*)', queue):
                output['input'] = extract_rm(line, r'(input=)(.*)', queue)

                continue
            if extract_rm(line, r'(executable=)(.*)', queue):
                output['executable'] = extract_rm(line, r'(executable=)(.*)', queue)
                continue
